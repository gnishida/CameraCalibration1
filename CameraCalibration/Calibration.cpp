...
void Calibration::rodrigues(cv::Mat& src, cv::Mat& dst) {
	if (src.cols == 1) {
		...
		cv::Mat tmp = src / theta;

		dst = cv::Mat::zeros(3, 3, CV_64F);
		dst.at<double>(0, 1) = -tmp.at<double>(2, 0);
		dst.at<double>(0, 2) = tmp.at<double>(1, 0);
		dst.at<double>(1, 0) = tmp.at<double>(2, 0);
		dst.at<double>(1, 2) = -tmp.at<double>(0, 0);
		dst.at<double>(2, 0) = -tmp.at<double>(1, 0);
		dst.at<double>(2, 1) = tmp.at<double>(0, 0);

		dst = dst * sin(theta) + cv::Mat::eye(3, 3, CV_64F) * cos(theta) + tmp * tmp.t() * (1 - cos(theta));
	} else {
		double theta = acos((src.at<double>(0, 0) + src.at<double>(1, 1) + src.at<double>(2, 2) - 1) * 0.5);

		dst = cv::Mat(3, 1, CV_64F);
		cv::Mat tmp = (src - src.t()) * 0.5 * theta / sin(theta);
